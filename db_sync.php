...
function update_database($db) {
	...
	update_college_survivor_wrong_picks ( $db, 16 );
     ...
     // Database update for ATS type picks
     {
		...
		if (! is_null ( $row ['hscore'] ) & ! is_null ( $row ['ascore'] )) {
			...
			if (($row ['hscore'] + $row ['spread']) > $row ['ascore']) {
				$query2 = "update question set correctans = '" . $row ['hteamID'] . "' where questionID = '" . $questionID . "'";
				$result2 = $db->query ( $query2 );
			} elseif (($row ['hscore'] + $row ['spread']) < $row ['ascore']) {
				$query2 = "update question set correctans = '" . $row ['ateamID'] . "' where questionID = '" . $questionID . "'";
				$result2 = $db->query ( $query2 );
			} else {
				$query2 = "update question set correctans = '-1' where questionID = '" . $questionID . "'";
				$result2 = $db->query ( $query2 );
			}
		}
		...
		$query = "select max(game.KOtime) from game,team where game.hteamID = team.teamID and 
         team.league = '" . $row ['league'] . "' and game.weeknum = '" . $row ['weeknum'] . "'";
		$result = $db->query ( $query );
		$row = $result->FETCH_ASSOC ();
...
function update_pick($pickID, $db) {
	...
	if ($row ['picktype'] == "S-COL") {
		...
		if ((is_null ( $row ['pick'] ) | $row ['pick'] == 0) & strtotime ( $row ['locktime'] ) < now_time ()) {
			...
			// find relevant game
			$query = "select * from game where game.weeknum='" . $row ['weeknum'] . "' and (game.hteamID='" . $row ['pick'] . "' or game.ateamID='" . $row ['pick'] . "')";
			$gameresult = $db->query ( $query );
			$gamerow = $gameresult->FETCH_ASSOC ();
			
			if (! is_null ( $gamerow ['hscore'] ) & ! is_null ( $gamerow ['ascore'] )) {
				// find winner of game
				if ($gamerow ['hscore'] > $gamerow ['ascore']) {
					$gamewinner = $gamerow ['hteamID'];
				} else {
					$gamewinner = $gamerow ['ateamID'];
				}
	...
	if ($row ['picktype'] == "S-PRO" & is_null ( $row ['pick'] ) == 0 & $row ['pick'] != 0) {
		...
		if (! is_null ( $gamerow ['hscore'] ) & ! is_null ( $gamerow ['ascore'] )) {
			...
			if ($gamerow ['hscore'] > $gamerow ['ascore']) {
				$gamewinner = $gamerow ['hteamID'];
			} elseif ($gamerow ['ascore'] > $gamerow ['hscore']) {
				$gamewinner = $gamerow ['ateamID'];
			} else {
				$gamewinner = - 1;
				echo "test";
